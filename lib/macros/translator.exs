defmodule Translator do
  ...
  # deftranslations returns an AST for all dectionary keys, like the following:
  # [[
  #     [def(t("en", "flash.hello", bindings)) do
  #     (((("" <> "Hello ") <> to_string(Dict.fetch!(bindings, :first))) <> " ") <> to_string(Dict.fetch!(bindings, :last))) <> "!"
  #     end,
  #     def(t("en", "flash.bye", bindings)) do
  ...
  defp interpolate(string) do 
    ~r/(?<head>)%{[^}]+}(?<tail>)/
    |> Regex.split(string, on: [:head, :tail]) # will return an array like: ["Hello ", "%{first}", " ", "%{last}", "!"]
    |> Enum.reduce "", fn
      #one function with multiples bodies!!
      #paterrn match arguments staring with %{
      <<"%{" <> rest>>, acc ->
